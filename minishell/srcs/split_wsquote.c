...
static int		find_next_word(char const *s, size_t i)
{
	while (s[i] == ' ' ||  s[i] == '\t')
	{
		if (s[i] == '"' && ++i)
...
static int		find_end_word(char const *s, size_t i)
{
	while (s[i] && s[i] != ' ' && s[i] != '\t')
	{
		if (s[i] == '"' && ++i)
...
static size_t	get_nb_word(char const *s)
{
	size_t		i;
	size_t		nb_word;
...
char			**split_wsquote(char const *s)
{
	...
	i = 0;
	j = 0;
	nb_word = !s ? 0 : get_nb_word(s);
	tab = !s ? 0 : (char **)malloc(sizeof(char *) * (nb_word + 1));
	if (!tab)
		return (NULL);
	while (j < nb_word)
	{
		i = find_next_word(s, i);
		tab[j] = ft_strsub(s, i, find_end_word(s, i) - i);
		if (!tab[j])
		{
			free_tab(tab, j);
			return (NULL);
